...
class Annotator:
	...
	def getAnnotations(self,sentence,dep_parse=False):
		...
		for senna_tag in senna_tags[0:-2]:
			if(not any(senna_tag)): return default
			senna_tag=senna_tag.split("\t")
...
def srl_extract(text):
	cw = os.getcwd()   
	os.chdir('/home/viswanath/Downloads/practNLPTools-1.0/practnlptools')   
	annotator=Annotator()
	...
	for line in lines:
		...
		temp = annotator.getAnnotations(line)["srl"]
	...
	for q in lst:
		if 'V'in q: key = q["V"]
		if 'A0' in q: VA0[key] = q["A0"].split()
		if 'A1' in q: VA1[key] = q["A1"].split()
