...
#train model based on top N wwords from TFIDF model	
	def train_model(self, dirname, w2v_model_path,topN,ndim):
		...
		#	print "Processing = " + fname
			...
			pword = self.top_n_words_doc( w2v_model_path,text,tfidf_model,topN)
...
# find top N words from TFIDF model
	...
	def get_docvec(self,w2v_model,tfidf_model, pos_words, text,topN,neg_fact=1,neg_words=[]):
		...
		tokens = text.split()
		X1 = [ w2vModel[i] for i in pos_words if i in tfidf_model_vocab if i in w2vModel.vocab]
		if len(neg_words) == 0:
			n_neg = topN*neg_fact
#			n_neg = 200
#			sim_pos_words = [x[0] for x in w2vModel.most_similar_cosmul(pos_words, topn=200)]
		...
		X2 = [ w2vModel[i] for i in neg_words if i in tfidf_model_vocab]
		X = X1 + X2
...
# For extracting data for train set
	...
	def NFoldTest(self,total_dirname, iter_N=5,split =0.30,random_state=0):
		...
		fn_test = []
		timestr = time.strftime("%Y_%m_%d_%H%M%S")
		fnRes = "Output_topN2_" + str(self.topN)+"_split_"+str(split*100) + "_mostSim_more_individual_" + timestr +".tsv"
		fnRes = res_dir + "/" + fnRes
		fp = open(fnRes,"wb")
...
if __name__ == '__main__': 
	...
	for topN in topNA:
		...
		gt.NFoldTest(train_dirname, iter_N=50,split =0.27)
	timestr = time.strftime("%Y_%m_%d_%H%M%S")
#	gt.printResult()
